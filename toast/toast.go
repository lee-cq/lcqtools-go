...
// Code From https://github.com/go-toast/toast/blob/master/toast.go
// Reduce the use of third-party libraries and no longer use UUIDs
// but instead the current time millisecond timestamp
...
func invokeTemporaryScript(content string) error {
	...
	file := filepath.Join(os.TempDir(), fmt.Sprintf("%d", id)+".ps1")
